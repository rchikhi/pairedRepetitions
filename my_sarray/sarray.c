...
static	void	qsort2(my_int*, my_int*, my_int n);
static	my_int	ssortit(my_int a[], my_int p[], my_int n, my_int h, my_int *pe, my_int nbuck);

my_int
sarray(my_int a[], my_int n)
{
	my_int i, l;
	my_int c, cc, ncc, lab, cum, nbuck;
	my_int k;
	my_int *p = 0;
	my_int result = -1;
	my_int *al;
...
my_int
bsarray(const uchar buf[], my_int p[], my_int n)
{
	my_int *a, buckets[256*256];
	my_int i, last, cum, c, cc, ncc, lab, id, nbuck;

	a = malloc((n+1)*sizeof(my_int));
	if(a == 0)
		return -1;
...
static my_int
ssortit(my_int a[], my_int p[], my_int n, my_int h, my_int *pe, my_int nbuck)
{
	my_int *s, *ss, *packing, *sorting;
	my_int v, sv, vv, packed, lab, t, i;
 ...
 */
static void
vecswap2(my_int *a, my_int *b, my_int n)
{
	while (n-- > 0) {
        	my_int t = *a;
		*a++ = *b;
...
static my_int*
med3(my_int *a, my_int *b, my_int *c, my_int *asucc)
{
	my_int va, vb, vc;
...
static void
inssort(my_int *a, my_int *asucc, my_int n)
{
	my_int *pi, *pj, t;
...
static void
qsort2(my_int *a, my_int *asucc, my_int n)
{
	my_int d, r, partval;
	my_int *pa, *pb, *pc, *pd, *pl, *pm, *pn, t;
